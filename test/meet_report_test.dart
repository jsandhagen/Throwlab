...
void main() {
  ...
  test('writes the series round by round, in standings order', () {
    ...
    final ana = lines.firstWhere((line) => line.contains('Ana Diaz'));
    final rival = lines.firstWhere((line) => line.contains('B. Rival'));
  ...
  test('a field too long for one page runs onto the next', () {
    ...
    // Two pages, and everybody on one of them.
    expect(String.fromCharCodes(bytes).contains('/Count 2'), isTrue);
    final text = pdfText(bytes)!;
    expect(text, contains('Thrower 0'));
    expect(text, contains('Thrower 59'));
    expect(text, contains('page 1 of 2'));
  });
